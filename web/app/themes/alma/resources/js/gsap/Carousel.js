...
class DragScroll {
  ...
  calculate() {
    ...
    this.maxScroll = this.wrapWidth - this.$el.clientWidth + (this.$items[0].clientWidth); // Add some space at the end
  }
  ...
  raf() {
    ...
    if (this.speed !== this.oldX - this.x) {
      ...
      this.$items.forEach((item) => {
        ...
        gsap.to(item, { scale: scale, duration: 0.1, overwrite: true });
      });
    }
  }
...
export const Carousel = {
  init() {
    ...
    console.log("gsap carousel started");
